...
- **阶段③④ 合流快进（2026-09-13，成品优先模式）**：包体 **92 mod**（95 pw.toml 记录，3 件摘除见下）。Symbiote 1.1.3、Sword Soaring 20.14.2.8+Invincible Lib 20.14.8.2、Nightfall 3.4.0+Avalon+AAA Particles、Resurrection 0.20.12.2（IF 实证版）、Indestructible 20.13.0（IF 实证版）、Spore 2.2.0j+Inquisition 3.1、Cataclysm 3.31、暮色森林 4.3.2508、Alex 双件、Iron's Spells 3.16.3、Tinkers 3.12.0.220、Saint's Dragons 0.9.6、Terralith+TerraBlender、YUNG×4、RoadWeaver、农业三件、Sophisticated 双件、Xaero 双件、FTB 任务四件、KubeJS+CrT+OpenLoader+GameStages 系、性能八件（entityculling/ImmediatelyFast/Clumps/memoryleakfix/packet-fixer/cull-leaves/dynamic-fps/chunksending）、手感三件（leawind 2.2.0 钉版/notenoughanimations/dummmmmmy）。**摘除记录**：impactful（Modrinth 仅 20.6 系，不兼容 EF 20.14.17）、Battle Arts+API（需 ex_cap API——EF 20.14.16-beta 有、20.14.17 移除了，降级与否待用户拍板）、Antique Atlas 4（Modrinth 1.20.1 为 Fabric+Connector 形态，探索引导暂由 RoadWeaver+Xaero+指南针承接）、Krypton（无 1.20.1 Forge 版）、Epic Foes（CF-only 且本地无实证 jar，暂缓）
- **IF 成品借用**：imfdata 魔改数据包 1992 文件（openloader 通道，含 saintsdragons 群系标签注入等）、IF 配置 57 件（暂存 `config/if_borrowed/` 待人工筛选平铺）、IMF-Trans/Trail/Modify 资源包（实例已启用，**发布合规待人工拍板**——这些是 IF 作者的劳动成果）
- **冒烟验证（2026-09-13 21:21）**：92 mod 启动到主菜单 ✅（166.9s，Nightfall shaders 正常加载；早显窗改走 config/fml.toml earlyWindowControl=false）
