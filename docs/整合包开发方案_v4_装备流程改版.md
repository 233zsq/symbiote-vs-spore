...
**结论：1.20.1 + Forge 47.2+。** 具体推荐 47.2.x 之后的稳定构建（修复了早期 47.x 的若干网络与事件总线问题）。1.21.1 + NeoForge 组合作为未来的远期升级路线保留观察，不在本期范围。（2026-09-10 落实：选定当时最新稳定构建 **47.4.5** 并冻结于 `pack.toml`——47.x 已进入纯维护期，模组在 47 系列内向后兼容良好；后续任何升级仍按红线走人工决策 + 全量回归测试。）
