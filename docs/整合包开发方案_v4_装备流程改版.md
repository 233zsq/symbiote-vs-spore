...
**结论：1.20.1 + Forge 47.2+。** 具体推荐 47.2.x 之后的稳定构建（修复了早期 47.x 的若干网络与事件总线问题）。1.21.1 + NeoForge 组合作为未来的远期升级路线保留观察，不在本期范围。（2026-09-10 落实：选定当时最新稳定构建 **47.4.23** 并冻结于 `pack.toml`——47.x 已进入纯维护期，模组在 47 系列内向后兼容良好；后续任何升级仍按红线走人工决策 + 全量回归测试。）
...
- **按键冲突处理**：EF 默认 **R 键=战斗/冒险模式切换、K 键=技能编辑菜单**，而 Symbiote 默认 **R 键=触须猛拉、K 键=掠食者形态**，直接撞键。解决方案已定型（2026-09-10）：用 **Default Options**（与 3.7 的 Xaero's 配置下发同一通道）把键位默认表 `config/defaultoptions/keybindings.txt` 随包下发——只对新安装生效、不覆盖玩家改键；EF 六键已入表（切战斗=mouse.5、固有技能=R、闪避=LAlt、格挡=右键、锁定=G、技能编辑器=K），**Symbiote 入包后将其键位改绑（拟 V=触须猛拉、B=掠食者形态，ID 以 Controlling 现场为准）追加进同一文件**；EF 键位保持上表以降低玩家学习成本；任务序章「觉醒」中以图文说明最终键位表。
...
| ②.5 开发环境补齐（2026-09-10 增设） | 后续实测在最终环境中进行 | 性能五件套（Embeddium、FerriteCore、ModernFix、spark、Chunky）与开发 QoL（JEI、Jade、Controlling、AppleSkin）提前入包——原排第四批的性能栈前移，避免后期入包使手感/兼容结论返工；spark 自此常驻支撑 Spore TPS 监控；同步定型键位下发机制（Default Options，`config/defaultoptions/` 默认表随包、不覆盖玩家改键） | 实例启动验证通过（含 Embeddium×EF 渲染实测） |
